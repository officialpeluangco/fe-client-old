...
const Header: React.FC<HeaderProps> = ({ white }) => {
    ...
    return (
        <header
            ...
            <ul className={`absolute md:static top-[60px] py-4 md:py-0 md:bg-transparent bg-white w-full md:flex-row flex-col items-center gap-2 justify-end list-none text-sm transition-all ${isOpen ? `flex` : `hidden`} ${white && scrollPosition == 0 ? 'md:text-white' : 'md:text-black'}`}>
            ...
            <button onClick={toggleMenu} className={`mr-4 ${scrollPosition > 0 || white === false ? 'text-black' : 'text-white'}`}>
                {isOpen ? <X /> : <Menu />}
            </button>
        </header>
    )
